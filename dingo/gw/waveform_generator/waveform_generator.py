...
def generate_FD_waveform(
        domain: Union[FrequencyDomain, TimeDomain], parameters_lal: Tuple
) -> PolarizationDict:
    """
    ...
    check_floats = all(
        map(lambda x: isinstance(x, float), parameters_lal[:18]))
    ...
    if not isclose(domain.delta_f, hp.deltaF, rel_tol=1e-6):
        raise ValueError(
            f"Waveform delta_f is inconsistent with domain: {hp.deltaF} vs {domain.delta_f}!"
            f"To avoid this, ensure that f_max = {domain.f_max} is a power of two"
    ...
    dt = 1 / hp.deltaF + (hp.epoch.gpsSeconds +
...
class Transform(Protocol):
    def __call__(self, pol: PolarizationDict) -> PolarizationDict:
        ...
...
class WaveformGenerator:
    ...
    def __init__(
        ...
        mode_list: Optional[Iterable[Tuple[str, int]]] = None,
        ...
        **kwargs,
        ...
        if not isinstance(approximant, str):
            raise ValueError(
                "approximant should be a string, but got", approximant)
        else:
            ...
            if "SEOBNRv5" not in approximant:
                ...
                if mode_list is not None:
                    ...
                    def _setup_mode_array(mode_list: Iterable[Tuple[str, int]]) -> lal.Dict:
                    ...
                    self.lal_params = _setup_mode_array(mode_list)
    ...
    @property
    def spin_conversion_phase(self) -> Optional[float]:
        return self._spin_conversion_phase

    @spin_conversion_phase.setter
    def spin_conversion_phase(self, value: Optional[float]) -> None:
        if value is None:
            print(
    ...
    def generate_hplus_hcross(
        ...
        parameters_generator = self._convert_parameters(
        ...
        wf_dict: PolarizationDict
        try:
            if isinstance(self.domain, FrequencyDomain):
                wf_dict = generate_FD_waveform(
                    self.domain, parameters_generator)
            elif isinstance(self.domain, TimeDomain):
                wf_dict = generate_TD_waveform(parameters_generator)
            ...
            if not catch_waveform_errors:
                ...
                if EDOM:
                    ...
                    wf_dict = PolarizationDict(
                        h_plus=pol_nan, h_cross=pol_nan
                    )
                else:
                    raise
    ...
    def generate_hplus_hcross_m(
        ...
        if not isinstance(parameters, dict):
            raise ValueError(
                "parameters should be a dictionary, but got", parameters)
        elif not isinstance(list(parameters.values())[0], float):
            raise ValueError(
                "parameters dictionary must contain floats", parameters)
        ...
        if isinstance(self.domain, FrequencyDomain):
            ...
            if LS.SimInspiralImplementedFDApproximants(self.approximant):
                ...
                assert LS.SimInspiralImplementedTDApproximants(
                    self.approximant)
                # Step 1: generate waveform modes in L0 frame in native domain of
                # approximant (here: TD)
...
if __name__ == "__main__":
    ...
    pol_ref = wfg.generate_hplus_hcross(
